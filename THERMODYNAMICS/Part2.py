...
while(pf<=end):
    ...
    val2 = ((b/v_g) - (a*U_g/v_g))

    coeff = (b*(1/v_f - 1/v_g) - a*(U_f/v_f - U_g/v_g))
    
    af = (val1 - val2)/coeff
    Vacc = A(m1, m2, t)/(g_1(pf, af) - g_1(pi, ai))
    
    if abs(Vacc) < abs(Vacc_min):
        Vacc_min = abs(Vacc)
        af_final = af
        pf_final = pf
...
print("Vacc_min = " + str(Vacc_min) + " m^3")
print("af = " + str(af_final))
print("Pf = " + str(pf_final) + " kPa")